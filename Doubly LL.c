...
struct node *insert_at_beginning(struct node *head, int data)
{
	...
	if(head)
	    ...
	    temp->next=NULL;//If no ele present in ll
	head=temp;
	return head;
...
//CASE 3 -	/*Deletion in between*/
	temp=head->next; // as first node deletion handled above
	while(temp->next!=NULL )// means until second last node
	{
		if(temp->info==data)     
		{
